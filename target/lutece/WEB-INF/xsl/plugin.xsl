...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
                    ...
                    <tr>
                        <td>
                            <table cellpadding="1" cellspacing="1" border="0" width="100%">
                                <tr>
                                    <th class="admin-title" height="30" colspan="3"><img src="../../{icon-url}" vspace="10" hspace="10" align="middle"/> Plugin : <xsl:value-of select="name" /></th>
                                </tr>
                                <tr>
                                    <td>
                                        <blockquote>
                                            ...
                                            <b>Auteur : </b><xsl:value-of select="provider" /><br/>
                                            ...
                                            <xsl:if test="not(string(documentation)='')">
                                                ...
                                                <xsl:value-of select="documentation" /><br/><br/>
                                            </xsl:if>
                                            <xsl:if test="not(string(installation)='')">
                                                ...
                                                <xsl:value-of select="installation" /><br/><br/>
                                            </xsl:if>
                                            <xsl:if test="not(string(changes)='')">
                                                ...
                                                <xsl:value-of select="changes"/><br/><br/>
                                            </xsl:if>
                                            <xsl:if test="not(string(user-guide)='')">
                                                ...
                                                <xsl:value-of select="user-guide" /><br/><br/>
                                            </xsl:if>
                                        </blockquote>
                                    </td>
                                </tr>
    ...
    <xsl:template match="admin-feature">
        <ul><li><xsl:value-of select="feature-title" /><br/>
        <i><xsl:value-of select="feature-description" /></i></li></ul>
    </xsl:template>
    ...
    <xsl:template match="portlet">
        <ul><li><xsl:value-of select="portlet-type-name" /></li></ul>
    </xsl:template>
    
    <xsl:template match="content-service">
        <ul><li>Ce plugin contient un service de Contenu basé sur la classe : <br/>
        <i><xsl:value-of select="content-service-class" /></i></li></ul>
    </xsl:template>
    
    <xsl:template match="link-service">
        <ul><li>Ce plugin contient un service de Contenu basé sur la classe : <br/>
        <i><xsl:value-of select="link-service-class" /></i></li></ul>
    </xsl:template>
    
    <xsl:template match="html-service">
        <ul><li>Ce plugin contient un service de Contenu basé sur la classe : <br/>
        <i><xsl:value-of select="html-service-class" /></i></li></ul>
    </xsl:template>
    ...
    <xsl:template match="param">
        <ul><li><b><xsl:value-of select="param-name" /></b> : <xsl:value-of select="param-value" /><br/><i><xsl:value-of select="param-description" /></i></li></ul>
    </xsl:template>
